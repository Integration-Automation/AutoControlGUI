...
#: The fixed-width numeric types, as ``struct`` format and byte width. Each is
#: aligned to its own width on the wire.
#:
#: ``i`` matters more than it looks: AT-SPI reports a component's extents as
#: four *signed* integers, because a window on a monitor left of or above the
...
#: ``h`` (UNIX_FD) is deliberately absent. On the wire it is a 32-bit index
#: into a descriptor array carried out-of-band in the message's control data,
#: and this client passes no control data — see the module docstring. Reading
#: one would hand a caller an index into an array that was never received, so
#: it stays an error that says so.
...
class _Writer:
    ...
    def fixed(self, code: str, value: Any) -> None:
...
class _Reader:
    ...
    def fixed(self, code: str) -> Any:
        """Read one fixed-width number, aligned to its own width."""
        fmt, width = _FIXED[code]
        self.align(width)
