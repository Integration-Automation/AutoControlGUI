...
*距離*度量(平方距離,而非相關——相關會把絕對色相正規化掉,使紅→綠邊與黑→藍邊
得分相同)進行比對,定位灰階比對會塌掉的色彩辨識目標。分數是 ``1 -`` 均方根距離,每個通道各以其
完整範圍正規化;色相沿色環取較短的一邊比較,所以色相 1 與 179 都是紅色。

本功能重用 ``color_region`` 的 RGB 載入器與 ``visual_match`` 的 resize / NMS / ``Match``。
``haystack`` 可注入;搜尋可在合成陣列上單元測試。不匯入 ``PySide6``。

註:*平坦*的單色 patch 會比對到每一塊同色區域——純色 blob 用 ``find_color_region`` 較合適;
``color_match`` 適用於有色彩*結構*的目標。
