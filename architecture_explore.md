...
| `utils/scheduler/` | 373 | 間隔式與 cron 式的 action JSON 排程器 |
| `utils/session_guard/` | 62 | 驅動輸入前先偵測工作階段是否已鎖定／非互動 |
| `utils/triggers/` | 1,156 | 事件驅動觸發引擎：影像／視窗／像素／檔案／webhook／IMAP 郵件 |
...
| `utils/mcp_server/` | 17,530 | **無頭 MCP 伺服器**（16K LOC，預設註冊 676 個工具＝657 個 `ac_*` + 19 個別名）：stdio + HTTP 傳輸、工具工廠與處理器、資源、prompt、稽核、限流、外掛熱重載 |
...
| `utils/socket_server/` | 134 | 執行 action JSON 的執行緒式 TCP 指令伺服器（預設綁 127.0.0.1） |
...
| `utils/generate_report/` | 308 | HTML／JSON／XML 三種報表產生器（Template Method） |
...
| `utils/test_suite/` | 443 | QA 套件編排：把扁平 action list 評分為測試案例 + CI 報表 |
...
#### `utils/executor/`（9,155 行）— 執行核心

| 檔案 | 行數 | 職責 |
| --- | ---: | --- |
| `action_executor.py` | 8,161 | `Executor` 類別與 `event_dict` 分派表（773 個指令），另含數百個把 utils 能力接成指令的 adapter 函式；全域單例 `executor` 與 `add_command_to_executor()` 擴充點。 |
...
| `flow_data_commands.py` | 254 | `AC_*_to_var` 資料來源與轉換指令：shell、時鐘、亂數、PDF、TOTP、SQL、檔案、HTTP、OCR，加上 `AC_assert_var`／`AC_assert_db`／`AC_assert_duration`／`AC_transform_var`。都不執行巢狀 action list，所以沒有迴圈／分支語意。 |
...
#### `utils/mcp_server/`（17,530 行，676 個工具）— 最大子系統

| 檔案 | 行數 | 職責 |
| --- | ---: | --- |
| `tools/_factories.py` | 8,984 | 工具工廠：每個函式回傳一個領域的 `MCPTool` 清單（把 `AC_*` 能力包成 MCP 工具）。 |
...
| `http_transport.py` | 565 | MCP 的 HTTP 傳輸。 |
| `http_sessions.py` | 234 | MCP 的 HTTP 傳輸用的 session 身分:`Mcp-Session-Id` 註冊表,以及每個 session 那條常駐的 server→client SSE 串流。 |
| `_client_requests.py` | 245 | 伺服器主動送出的請求：`roots/list`／`elicitation/create`／`sampling/createMessage`,對應表與回應路由,以及破壞性工具的確認交握。 |
...
| `utils/hotkey/` | 7 | 738 |
| 其餘模組（約 286 個 `utils/` 子套件 + `android/`／`ios/`／周邊小工具） | 673 | 47,960 |
| **總計** | **1,037** | **142,252** |
