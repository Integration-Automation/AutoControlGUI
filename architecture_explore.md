...
| `utils/usb/` | 4,458 | 跨平台 USB 列舉／熱插拔／裝置直通（WinUSB、IOKit、libusb 後端 + ACL + WebRTC DataChannel 通道） |
...
#### `utils/usb/`（4,458 行）與 `utils/usbip/`（945 行）
...
| `utils/hotkey/` | 7 | 837 |
| 其餘模組（約 286 個 `utils/` 子套件 + `android/`／`ios/`／周邊小工具） | 676 | 50,572 |
| **總計** | **1,042** | **146,149** |
