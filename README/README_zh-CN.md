...
- **不写 Python 也能脚本化。** 758 个 `AC_*` 命令覆盖全部功能，因此一个 JSON 文件能做到库
...
| JSON 脚本 | `execute_action`、`execute_files` | 全部 758 个命令 | Script、Script Builder |
...
| 接口 | 启动方式 | 说明 |
|---|---|---|
| **MCP 服务器** | `je_auto_control_mcp`（stdio）或 `AC_start_mcp_http_server` | 664 个工具，供 Claude Desktop／Claude Code／自定义 tool loop 使用。Bearer 认证、TLS、审计日志、限流、插件热重载、CI 假后端。 |
